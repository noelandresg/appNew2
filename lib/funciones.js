  ...
  if (button === 2) {
    if (validaCabeceroDoc()) {
            ...
            //function pedido (cliente, establecimiento, fecha, notas, sucursal, usuario, productos)
             ...
             success = function(data) {
                 if (data[0] !== null) {
                     alertBox("La información se ha guardado correctamente. MSG: " + JSON.stringify(data[0]), "cbDefault()", "BMS Móvil", "OK");
                     pedidoGuardado = true;
             ...
             $.ajax({
                 type: 'POST',
                 url: url,
                 data: {"cliente":bmsCliente,"estab":bmsEstab,"fecha":bmsFecha,"notas":bmsNotas,"sucursal":bmsSucursal,"user":bmsUser,"productos":pedLineas},
                 dataType: "json",
                 crossDomain: false,
                 cache: false,
                 success: success,
